...
contract BENC is ERC1155, IERC20, IBENC {
    ...
    function createNFTcode(uint256 amount,string memory phonenumber,string memory email) public {      
...
function transferNFT(address from, address to , uint256 amount, 
string memory phonenumber,string memory email) public {   
    ...
    require(amount<=sellerBalance,"Amount want to transfer is greater than sellerBalance");
     ...
     for(uint256 x = 0;x<amount;x++){     
        uint256 length = tracker[from].length-1;
         uint256 trackNumber = tracker[from][length];  //get trackNumber         
